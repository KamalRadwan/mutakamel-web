...
export function StorageServersScreen() {
  ...
  if (!view.canRead) {
    return <section className="mx-auto max-w-xl rounded-2xl border border-rose-200 bg-rose-50 p-8 text-center text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-100"><AlertCircle className="mx-auto size-10" /><h1 className="mt-3 text-lg font-black">{isArabic ? "تم رفض الوصول" : "Access denied"}</h1><p className="mt-2 text-sm">{isArabic ? "لا تملك صلاحية قراءة خوادم التخزين." : "You do not have permission to read Storage Servers."}</p></section>;
  }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
        <div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_180px_220px_auto] lg:items-end">
          ...
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 px-4 text-sm font-bold hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:hover:bg-slate-900"
          >
      ...
      {view.error ? (
        <section role="alert" className="flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-100">
          ...
          <div><p className="font-bold">{isArabic ? "تعذر تحميل خوادم التخزين" : "Storage servers could not be loaded"}</p><p className="mt-1">{view.error.message}</p>{view.error.correlationId ? <p className="mt-1 font-mono text-xs">Correlation ID: {view.error.correlationId}</p> : null}</div>
        </section>
      ) : null}
...
function ServerRow({ server, isArabic }: { server: StorageServerView; isArabic: boolean }) {
  ...
  return (
    <tr className="align-top hover:bg-slate-50/70 dark:hover:bg-slate-900/50">
      <td className="px-5 py-4"><Link href={`/storage-servers/${server.id}`} className="font-bold text-indigo-700 hover:underline dark:text-indigo-300">{server.name}</Link><p className="mt-1 font-mono text-xs text-slate-500">{server.code}</p>{server.isPlatformDefault ? <span className="mt-2 inline-flex rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-bold uppercase text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300">{isArabic ? "الافتراضي" : "Platform default"}</span> : null}</td>
      ...
      <td className="px-5 py-4 font-mono font-bold">{server.assignedTenants} / {server.maxTenants ?? "∞"}</td>
      <td className={`px-5 py-4 ${evidenceTone}`}><p className="font-bold">{evidenceLabel(server, isArabic)}</p><p className="mt-1 text-xs text-slate-500">{formatDate(server.lastConnectionTestedAt, isArabic)}</p>{server.lastConnectionTestErrorCode ? <code className="mt-1 block max-w-xs truncate text-[11px]" title={server.lastConnectionTestErrorCode}>{server.lastConnectionTestErrorCode}</code> : null}</td>
...
function SummaryCard({ label, value, icon, tone }: { label: string; value: number; icon: ReactNode; tone: "indigo" | "emerald" | "amber" }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950"><div className="flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">{label}</p><span className={`grid size-8 place-items-center rounded-lg ${tones[tone]}`}>{icon}</span></div><p className="mt-2 font-mono text-2xl font-black">{value}</p></div>;
}
