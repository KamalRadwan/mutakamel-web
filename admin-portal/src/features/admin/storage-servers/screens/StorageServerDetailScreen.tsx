...
export function StorageServerDetailScreen({ id }: { id: string }) {
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-semibold">{server.name}</h1><StatusBadge status={server.status} enumType="db-server" />{server.isPlatformDefault ? <span className="rounded-full bg-indigo-50 px-2.5 py-1 text-2xs font-semibold uppercase text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300">{isArabic ? "الافتراضي" : "Platform default"}</span> : null}</div>
...
function StorageServerEditor({ mode, server, isArabic, connectionEditBlocked, isSubmitting, onClose, onSubmit }: { mode: "configuration" | "credentials"; server: StorageServerView; isArabic: boolean; connectionEditBlocked: boolean; isSubmitting: boolean; onClose: () => void; onSubmit: (dto: UpdateStorageServerDto) => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4" role="presentation" onMouseDown={onBackdropMouseDown}>
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} tabIndex={-1} onKeyDown={onKeyDown} className="w-full max-w-xl rounded-xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-start justify-between gap-3"><div><h2 id={titleId} className="text-lg font-semibold">{title}</h2><p id={descriptionId} className="mt-1 text-xs leading-5 text-slate-500">{description}</p></div><button type="button" onClick={onClose} disabled={isSubmitting} aria-label={isArabic ? "إغلاق" : "Close"} className="grid size-11 place-items-center rounded-xl hover:bg-slate-100 disabled:opacity-50 dark:hover:bg-slate-900"><X className="size-4" /></button></div>
...
function ProbeResultBanner({ result, isArabic }: { result: { outcome: string; errorCode: string | null; lifecycleStatus: string }; isArabic: boolean }) { const passed = result.outcome === "PASSED"; return <section role="status" className={`rounded-xl border p-4 text-sm ${passed ? "border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-100" : "border-rose-200 bg-rose-50 text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-100"}`}><p className="font-semibold">{passed ? isArabic ? "نجح اختبار الاتصال" : "Connection test passed" : isArabic ? "فشل اختبار الاتصال" : "Connection test failed"}</p><p className="mt-1 text-xs">{isArabic ? `لم تتغير حالة دورة الحياة: ${result.lifecycleStatus}.` : `Lifecycle state remains ${result.lifecycleStatus}.`}{result.errorCode ? ` · ${result.errorCode}` : ""}</p></section>; }
function Panel({ title, icon, action, children }: { title: string; icon: ReactNode; action?: ReactNode; children: ReactNode }) { return <section className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950"><div className="flex min-h-14 items-center justify-between gap-3 border-b border-slate-200 px-5 dark:border-slate-800"><h2 className="flex items-center gap-2 text-sm font-semibold"><span className="text-indigo-600 dark:text-indigo-400">{icon}</span>{title}</h2>{action}</div><div className="p-5">{children}</div></section>; }
function Datum({ label, value, mono, wide }: { label: string; value: string; mono?: boolean; wide?: boolean }) { return <div className={wide ? "sm:col-span-2" : ""}><dt className="text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</dt><dd className={`mt-2 break-all text-sm font-semibold ${mono ? "font-mono" : ""}`}>{value}</dd></div>; }
...
function PolicyRow({ label, value, good }: { label: string; value: string; good: boolean }) { return <div className="flex items-center justify-between gap-4 rounded-xl bg-slate-50 p-3 text-xs dark:bg-slate-900"><span className="text-slate-500">{label}</span><span className={`text-end font-semibold ${good ? "text-emerald-700 dark:text-emerald-300" : "text-amber-700 dark:text-amber-300"}`}>{value}</span></div>; }
function EditorField({ label, wide, children }: { label: string; wide?: boolean; children: ReactNode }) { return <label className={`block ${wide ? "sm:col-span-2" : ""}`}><span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</span>{children}</label>; }
function AccessDenied({ isArabic }: { isArabic: boolean }) { return <section className="mx-auto max-w-xl rounded-xl border border-rose-200 bg-rose-50 p-8 text-center text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-100"><AlertCircle className="mx-auto size-10" /><h1 className="mt-3 text-lg font-semibold">{isArabic ? "تم رفض الوصول" : "Access denied"}</h1></section>; }
...
function ErrorState({ message, isArabic }: { message: NormalizedApiError | null; isArabic: boolean }) { return <section className="mx-auto max-w-xl rounded-xl border border-rose-200 bg-rose-50 p-8 text-center text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-100"><AlertCircle className="mx-auto size-10" /><h1 className="mt-3 text-lg font-semibold">{isArabic ? "تعذر تحميل الخادم" : "Storage server unavailable"}</h1><p className="mt-2 whitespace-pre-line text-sm">{message ? readErrorMessage(message, "") : null}</p><Link href="/storage-servers" className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-white px-4 font-semibold text-rose-800">{isArabic ? "العودة" : "Back"}</Link></section>; }
...
const primaryActionClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50";
const secondaryActionClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-semibold hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:bg-slate-950 dark:hover:bg-slate-900";
const dangerOutlineClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-rose-300 px-4 text-sm font-semibold text-rose-700 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-45 dark:border-rose-800 dark:text-rose-300 dark:hover:bg-rose-950/40";
const smallButtonClass = "inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-xs font-semibold text-indigo-700 hover:bg-indigo-50 disabled:cursor-not-allowed disabled:opacity-45 dark:text-indigo-300 dark:hover:bg-indigo-950/40";
