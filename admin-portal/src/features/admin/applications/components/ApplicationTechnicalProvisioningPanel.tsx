    ...
    >
      <header className="flex flex-col justify-between gap-3 border-b border-slate-200 bg-slate-950 px-5 py-4 text-white sm:flex-row sm:items-center dark:border-slate-800">
        <div>
          ...
          <h2 id="technical-readiness-title" className="mt-1 flex items-center gap-2 text-sm font-semibold">
        ...
        <button
          ...
          className="inline-flex min-h-11 self-start items-center gap-2 rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-xs font-semibold hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:opacity-50"
        >
          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold">{copy.statuses[readiness.status]}</div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <code className="rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-200" dir="ltr">
                  {applicationKey}
                </code>
                <code className="rounded-lg bg-cyan-50 px-2.5 py-1.5 text-xs font-semibold text-cyan-800 dark:bg-cyan-950/40 dark:text-cyan-200" dir="ltr">
            ...
            {readiness.selectionBlockers.length > 0 && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/60">
                <h3 className="text-xs font-semibold">{copy.selectionTitle}</h3>
                <ul className="mt-2 grid gap-2 text-xs text-slate-600 sm:grid-cols-2 dark:text-slate-300">
            ...
            {readiness.components.map((component) => (
              <article key={component.id} className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="font-mono text-xs font-semibold text-cyan-700 dark:text-cyan-300" dir="ltr">{component.key}</p>
                    <p className="mt-1 text-xs text-slate-500">{component.kind} · {copy.contractShort} {component.contractVersion}</p>
                  </div>
                ...
                <p className="mt-3 text-xs text-slate-500">{copy.noCredential}</p>
              </article>
            ))}
            ...
            {showAdoptionAction && (
              <div className="flex flex-col justify-between gap-3 rounded-xl border border-violet-200 bg-violet-50 p-4 sm:flex-row sm:items-center dark:border-violet-900 dark:bg-violet-950/20">
                <div>
                  <h3 className="text-xs font-semibold text-violet-950 dark:text-violet-100">{copy.adoptionCalloutTitle}</h3>
                  <p className="mt-1 text-xs leading-relaxed text-violet-800 dark:text-violet-300">{copy.adoptionCalloutDescription}</p>
                  {!canManage && (
                    <p className="mt-2 font-mono text-xs text-amber-700 dark:text-amber-300">{copy.permissionRequired}</p>
                  )}
                </div>
                {canManage && (
                  <button
                    ...
                    className="min-h-11 shrink-0 rounded-xl bg-violet-700 px-4 py-2 text-xs font-semibold text-white hover:bg-violet-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900"
                  >
                    {copy.adoptIdentity}
                  </button>
                )}
            ...
            {showBindingAction && (
              <div className="flex flex-col justify-between gap-3 rounded-xl border border-cyan-200 bg-cyan-50 p-4 sm:flex-row sm:items-center dark:border-cyan-900 dark:bg-cyan-950/20">
                <div>
                  <h3 className="text-xs font-semibold text-cyan-950 dark:text-cyan-100">{copy.bindingCalloutTitle}</h3>
                  <p className="mt-1 text-xs leading-relaxed text-cyan-800 dark:text-cyan-300">{copy.bindingCalloutDescription}</p>
                  {!canManage && (
                    <p className="mt-2 font-mono text-xs text-amber-700 dark:text-amber-300">{copy.permissionRequired}</p>
                  )}
                </div>
                {canManage && (
                  <button
                    ...
                    className="min-h-11 shrink-0 rounded-xl bg-cyan-700 px-4 py-2 text-xs font-semibold text-white hover:bg-cyan-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-600 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900"
                  >
                    {copy.linkComponent}
                  </button>
                )}
...
function Check({ label, value, icon: Icon, passed, blocked }: { label: string; value: boolean; icon: typeof Boxes; passed: string; blocked: string }) {
  return (
    <div className={`rounded-xl border p-3 ${value ? "border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/20" : "border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/20"}`}>
      ...
      <div className="mt-1 text-2xs uppercase tracking-wider text-slate-500">{value ? passed : blocked}</div>
    </div>
  );
}
...
function ChainStep({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-950">
      ...
      <div className="mt-1 break-all font-mono text-xs font-semibold text-slate-900 dark:text-slate-100">{value}</div>
    </div>
  );
}
