...
export function ApplicationLifecycleDialog({ action, currentStatus, isSubmitting, onClose, onConfirm }: Props) {
  ...
  return (
    <div role="presentation" onMouseDown={onBackdropMouseDown} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} tabIndex={-1} onKeyDown={onKeyDown} className="w-full max-w-md rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
        <header className="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800">
          <div>
            <h2 id={titleId} className="text-sm font-semibold">{t.applications.detail.lifecycle[action]} {t.applications.detail.lifecycle.titleSuffix}</h2>
        ...
        <form onSubmit={submit} className="space-y-4 p-5">
          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">{t.applications.detail.lifecycle.reason}<textarea required maxLength={256} rows={3} value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1.5 w-full resize-none rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-violet-500 dark:border-slate-700 dark:bg-slate-950" /></label>
