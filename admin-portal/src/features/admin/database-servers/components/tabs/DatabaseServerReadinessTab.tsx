      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-md dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-4 border-b border-slate-200 bg-slate-50/80 p-5 dark:border-slate-800 dark:bg-slate-800/50 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-xs font-bold bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            {canBootstrapInitial &&
              ...
              server.credentialBootstrap.status !== "READY" && (
                <button
                  ...
                  title={d.retrySetup}
                  className="rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer shadow-md shadow-blue-500/20"
                >
                  {d.retrySetup}
                </button>
              )}
        ...
        <div className="space-y-4 p-5">
          ...
          {backupDependencyNeedsAttention && (
            <div
              role="alert"
              className="flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-950 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-200 sm:flex-row sm:items-center sm:justify-between"
            >
              <div>
                <p className="text-sm font-bold">{d.backupNeedsAttentionTitle}</p>
              ...
              {canReadBackup && (
                <Link
                  ...
                  className="inline-flex shrink-0 items-center justify-center rounded-xl border border-amber-300 bg-white px-3 py-2 text-xs font-bold text-amber-800 transition hover:bg-amber-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 dark:border-amber-800 dark:bg-slate-900 dark:text-amber-200 dark:hover:bg-amber-950"
                >
                  {d.openBackup}
                </Link>
              )}
          ...
          {provisioningPrincipal ? (
            <article className="rounded-2xl border border-slate-200 p-5 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850/40">
                ...
                {canRegenerate &&
                  ["READY", "DEFERRED"].includes(provisioningPrincipal.status) && (
                    <button
                      ...
                      className="rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 px-3 py-1.5 text-xs font-bold text-slate-800 dark:text-slate-200 transition cursor-pointer"
                    >
                      {d.rotatePassword}
                    </button>
                  )}
                ...
                {canReconcile &&
                  (provisioningPrincipal.status === "ROTATING" ||
                    ...
                    <button
                      ...
                      className="rounded-xl bg-amber-100 hover:bg-amber-200 px-3 py-1.5 text-xs font-bold text-amber-700 dark:bg-amber-950/40 dark:text-amber-300 transition cursor-pointer"
                    >
                      {d.reconcile}
                    </button>
                  )}
            ...
            <div
              role="alert"
              className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-rose-900 dark:border-rose-900/70 dark:bg-rose-950/30 dark:text-rose-200"
            >
              <p className="text-sm font-bold">{d.provisioningUnavailableTitle}</p>
