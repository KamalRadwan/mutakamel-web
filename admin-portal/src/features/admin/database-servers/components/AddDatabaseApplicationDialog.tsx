...
export function AddDatabaseApplicationDialog({ isOpen, boundApplicationKeys, isSubmitting, onClose, onBootstrap }: Props) {
  ...
  const available = useMemo(() => applications.filter((application) => !boundKeys.has(application.key) && application.databaseAccessMode === "TENANT_DATABASE"), [applications, boundKeys]);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      setError(null);
      setReason("");
      applicationsApi.list({ page: 1, limit: 100, lifecycleStatus: "ACTIVE", databaseAccessMode: "TENANT_DATABASE" })
        .then(({ data }) => {
          setApplications(data);
          setSelectedKey(data.find((application) => !boundKeys.has(application.key))?.key ?? "");
        })
