  ...
  return (
    <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-800">
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <label className="text-xs text-slate-500">Interval hours<input type="number" min={24} max={8760} value={editor.intervalHours} onChange={(event) => editor.setIntervalHours(Number(event.target.value))} disabled={!canUpdate || editor.pending} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
        <label className="text-xs text-slate-500">Window start UTC<input type="number" min={0} max={23} value={editor.windowStart} onChange={(event) => editor.setWindowStart(Number(event.target.value))} disabled={!canUpdate || editor.pending} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
        <label className="text-xs text-slate-500">Window hours<input type="number" min={1} max={24} value={editor.windowHours} onChange={(event) => editor.setWindowHours(Number(event.target.value))} disabled={!canUpdate || editor.pending} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
      </div>
      {canUpdate && (
