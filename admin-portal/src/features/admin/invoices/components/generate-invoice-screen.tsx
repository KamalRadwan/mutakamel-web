...
function GenerateForm({ generator, copy }: { generator: ReturnType<typeof useGenerateInvoice>; copy: InvoiceCopy }) {
  ...
  return (
    <section className="mx-auto w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <header className="border-b border-slate-200 bg-gradient-to-r from-indigo-50 to-white px-5 py-4 dark:border-slate-800 dark:from-indigo-950/40 dark:to-slate-900">
        <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-indigo-700 text-white"><FilePlus2 className="size-5" /></span><div><h1 className="text-xl font-black">{copy.generationTitle}</h1><p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">{copy.generationHelp}</p></div></div>
      </header>
      <form onSubmit={(event) => void generator.submit(event)} className="space-y-4 p-5" aria-label={copy.generationTitle}>
        ...
        <div className="flex justify-end border-t border-slate-200 pt-4 dark:border-slate-800">
          <button type="submit" disabled={pending} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-indigo-700 px-5 text-sm font-black text-white hover:bg-indigo-800 disabled:cursor-wait disabled:opacity-60">
...
function GeneratedInvoice({ generator, copy, lang }: { generator: ReturnType<typeof useGenerateInvoice>; copy: InvoiceCopy; lang: "ar" | "en" }) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-3">
      <section role="status" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-emerald-950 shadow-sm dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100">
        <h1 className="text-xl font-black">{copy.generated}</h1><p className="mt-1 text-sm">{copy.generatedHelp}</p>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {generator.permissions.canRead ? <Link href={`/invoices/${created.data.id}`} className="inline-flex min-h-11 items-center rounded-xl bg-emerald-800 px-4 text-sm font-black text-white hover:bg-emerald-900">{copy.viewGenerated}</Link> : null}
          <button type="button" onClick={generator.startAnother} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-emerald-300 px-4 text-sm font-bold"><RotateCcw className="size-4" />{copy.startAnother}</button>
        </div>
      </section>
...
function Evidence({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return <div className="rounded-xl bg-white/70 px-3 py-2 dark:bg-slate-950/40"><dt className="text-xs font-bold opacity-70">{label}</dt><dd dir={mono ? "ltr" : undefined} className={`mt-1 text-base font-black ${mono ? "font-mono text-start" : ""}`}>{value}</dd></div>;
}
