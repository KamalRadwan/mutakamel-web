    ...
    <div className="space-y-4" aria-busy={detail.isRefreshing || detail.mutation.phase === "PENDING"}>
      <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><div className="flex flex-wrap items-center gap-3"><h1 dir="ltr" className="font-mono text-xl font-semibold text-start sm:text-2xl">{invoice.number}</h1><InvoiceStatusBadge status={invoice.status} /></div><code dir="ltr" className="mt-2 block break-all text-start text-xs text-slate-500">{invoice.id}</code><p className="mt-2 font-mono text-xs font-semibold text-indigo-700 dark:text-indigo-300">{invoice.purpose}</p></div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={detail.refresh} disabled={detail.isRefreshing} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 px-3 text-sm font-semibold disabled:cursor-wait disabled:opacity-60 dark:border-slate-700"><RefreshCw className={`size-4 ${detail.isRefreshing ? "animate-spin" : ""}`} />{copy.refresh}</button>
            {detail.canEdit ? <button type="button" onClick={detail.openEdit} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-indigo-300 bg-indigo-50 px-3 text-sm font-semibold text-indigo-800 dark:border-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-200"><Pencil className="size-4" />{copy.editDraft}</button> : null}
            {detail.canIssue ? <button type="button" onClick={detail.openIssue} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-amber-600 px-4 text-sm font-semibold text-white hover:bg-amber-700"><FileCheck2 className="size-4" />{copy.issueInvoice}</button> : null}
            {detail.canVoid ? <button type="button" onClick={detail.openVoid} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-rose-700 px-4 text-sm font-semibold text-white hover:bg-rose-800"><Trash2 className="size-4" />{copy.voidInvoice}</button> : null}
          </div>
        </div>
      ...
      {detail.canRecordOfflinePayment ? (
        <section className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-950 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">{copy.offlinePayment}</h2><p className="mt-1 text-sm">{copy.offlinePaymentHint}</p></div><Link href={`/tenants/${invoice.tenantId}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-800 px-4 text-sm font-semibold text-white"><CircleDollarSign className="size-4" />{copy.offlinePayment}</Link></div>
        </section>
      ) : null}
...
function EvidenceCard({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900"><h2 className="text-base font-semibold">{title}</h2><dl className="mt-3 grid gap-2">{children}</dl></section>;
}
...
function Evidence({ label, value, mono, link }: { label: string; value: string; mono?: boolean; link?: string }) {
  ...
  return <div className="rounded-xl bg-slate-100 px-3 py-2 dark:bg-slate-800/70"><dt className="text-xs font-semibold text-slate-500 dark:text-slate-400">{label}</dt><dd dir={mono ? "ltr" : undefined} className={`mt-1 break-all text-sm font-semibold ${mono ? "font-mono text-start" : ""}`}>{output}</dd></div>;
}
  ...
  return (
    ...
    >
      <section
        ...
        tabIndex={-1}
        onKeyDown={onKeyDown}
        className="my-auto w-full max-w-5xl rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900"
      >
...
function DialogActions({ copy, pending, onCancel, submitLabel, danger }: { copy: InvoiceCopy; pending: boolean; onCancel: () => void; submitLabel: string; danger: boolean }) {
  return <div className="flex flex-wrap justify-end gap-2 border-t border-slate-200 pt-4 dark:border-slate-800"><button type="button" onClick={onCancel} disabled={pending} className="min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-semibold dark:border-slate-700">{copy.cancel}</button><button type="submit" disabled={pending} className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-5 text-sm font-semibold text-white disabled:cursor-wait disabled:opacity-60 ${danger ? "bg-rose-700 hover:bg-rose-800" : "bg-indigo-700 hover:bg-indigo-800"}`}>{pending ? <Loader2 className="size-4 animate-spin" /> : null}{submitLabel}</button></div>;
}
