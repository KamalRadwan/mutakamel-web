    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto_auto] xl:items-end">
          ...
          <button type="button" onClick={() => view.setQuery({ ...(runId ? { runId } : {}), ...(databaseServerId ? { databaseServerId } : {}), ...(tenantId ? { tenantId } : {}) })} disabled={invalidUuid} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-cyan-700 px-4 text-sm font-bold text-white hover:bg-cyan-800 disabled:opacity-50"><Filter className="size-4" />{isArabic ? "تطبيق" : "Apply"}</button>
      ...
      {view.isLoading ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
          <div className="overflow-x-auto"><table className="w-full min-w-[1040px] text-sm"><thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500 dark:bg-slate-900"><tr><th className="px-5 py-3 text-start">{isArabic ? "النسخة" : "Artifact"}</th><th className="px-5 py-3 text-start">{isArabic ? "قاعدة البيانات" : "Database"}</th><th className="px-5 py-3 text-start">{isArabic ? "الحجم" : "Size"}</th><th className="px-5 py-3 text-start">SHA-256</th><th className="px-5 py-3 text-start">{isArabic ? "الاكتمال" : "Finished"}</th><th className="px-5 py-3 text-start">{isArabic ? "الحالة" : "Status"}</th><th className="px-5 py-3 text-end">{isArabic ? "الإجراءات" : "Actions"}</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{view.artifacts.map((artifact) => {
            ...
            return <tr key={artifact.id} className="align-top hover:bg-slate-50/70 dark:hover:bg-slate-900/50"><td className="px-5 py-4"><div className="flex gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-cyan-50 text-cyan-700 dark:bg-cyan-950/50 dark:text-cyan-300"><FileArchive className="size-4" /></span><div><p className="font-mono font-bold" title={artifact.id}>{shortBackupId(artifact.id)}</p><p className="mt-1 font-mono text-xs text-slate-500" title={artifact.runId}>{isArabic ? "العملية" : "Run"}: {shortBackupId(artifact.runId)}</p></div></div></td><td className="px-5 py-4"><p className="font-mono font-bold">{artifact.databaseName}</p><p className="mt-1 font-mono text-xs text-slate-500" title={artifact.tenantId}>{shortBackupId(artifact.tenantId)}</p></td><td className="px-5 py-4 font-mono font-bold">{formatBackupBytes(artifact.sizeBytes, isArabic ? "ar-EG" : "en-US")}</td><td className="px-5 py-4 font-mono text-xs" title={artifact.sha256 ?? ""}>{artifact.sha256 ? `${artifact.sha256.slice(0, 12)}…` : "—"}</td><td className="px-5 py-4 text-slate-600 dark:text-slate-300">{formatBackupDate(artifact.finishedAt, isArabic ? "ar-EG" : "en-US")}</td><td className="px-5 py-4"><BackupStatusBadge status={artifact.status} />{artifact.hasFailure ? <p className="mt-2 max-w-xs text-xs text-rose-600 dark:text-rose-300">{isArabic ? "تم تسجيل تفاصيل الفشل بأمان في Worker." : "Failure details are retained in Worker logs."}</p> : null}</td><td className="px-5 py-4 text-end"><div className="inline-flex items-center gap-1">{view.canRestore && artifact.status === BackupArtifactStatus.COMPLETED ? <Link href={`/backup/restores?artifactId=${encodeURIComponent(artifact.id)}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-xs font-bold text-cyan-700 hover:bg-cyan-50 dark:text-cyan-300 dark:hover:bg-cyan-950/40"><ArchiveRestore className="size-4" />{isArabic ? "استعادة" : "Restore"}</Link> : null}{view.canDelete && terminal ? <button type="button" onClick={() => setDeletingArtifact(artifact)} className="grid size-11 place-items-center rounded-xl text-rose-700 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-950/40" aria-label={isArabic ? "حذف النسخة" : "Delete artifact"}><Trash2 className="size-4" /></button> : null}</div></td></tr>;
          })}</tbody></table></div>
