    ...
    <div className="space-y-6">
      <BackupPageHeader
        ...
        actions={view.canRestore ? (
          <button type="button" onClick={openStart} disabled={view.artifacts.length === 0 || Boolean(view.activeAction)} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-cyan-700 px-4 text-sm font-bold text-white hover:bg-cyan-800 disabled:cursor-not-allowed disabled:opacity-50"><Play className="size-4" />{isArabic ? "بدء اختبار استعادة" : "Start restore test"}</button>
        ) : undefined}
      />
      ...
      {view.retryableCommandError || view.pendingStartAttempt || view.pendingPromotionAttempt ? (
        <section role="status" className="rounded-2xl border border-amber-300 bg-amber-50 p-5 text-amber-950 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100">
          <h2 className="font-bold">{isArabic ? "هناك أمر استعادة لم تُحسم نتيجته" : "A restore command still has an unknown outcome"}</h2>
