    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
        <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          ...
          />
          <button type="button" onClick={() => void view.refreshBinding()} disabled={!view.selectedServerId || view.isLoadingBinding} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 px-4 text-sm font-bold hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:hover:bg-slate-900">
      ...
      {view.isLoading || view.isLoadingBinding ? (
        ...
        <>
          <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-wider text-slate-500">{isArabic ? "الحساب الثابت" : "Fixed principal"}</p>
                    <p className="mt-1 font-mono text-base font-bold">{view.binding.databasePrincipal}</p>
                  </div>
                </div>
              ...
              <dl className="mt-6 grid gap-4 sm:grid-cols-2">
                {[
                  ...
                  <div key={label} className="rounded-xl bg-slate-50 p-4 dark:bg-slate-900">
                    ...
                    <dd className="mt-1 break-all text-sm font-bold text-slate-900 dark:text-white">{value}</dd>
                  </div>
                ))}
              </dl>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-bold">{isArabic ? "سياسة التدوير" : "Rotation policy"}</h2>
              </div>
              <dl className="mt-5 space-y-3 text-sm">
                <div className="flex justify-between gap-4"><dt className="text-slate-500">{isArabic ? "الحالة" : "Status"}</dt><dd className="font-bold">{view.binding.rotationEnabled ? (isArabic ? "مفعلة" : "Enabled") : (isArabic ? "متوقفة" : "Disabled")}</dd></div>
                <div className="flex justify-between gap-4"><dt className="text-slate-500">{isArabic ? "الفترة" : "Interval"}</dt><dd className="font-bold">{view.binding.rotationIntervalHours}h</dd></div>
                <div className="flex justify-between gap-4"><dt className="text-slate-500">{isArabic ? "نافذة الصيانة" : "Maintenance window"}</dt><dd className="font-bold">UTC {view.binding.maintenanceWindowStartUtc}:00 +{view.binding.maintenanceWindowHours}h</dd></div>
              </dl>
              {view.canUpdatePolicy ? (
                <button type="button" onClick={() => setDialog("policy")} disabled={!view.ownsSelectedServerState || Boolean(view.activeAction)} className="mt-6 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-cyan-700 px-4 text-sm font-bold text-white hover:bg-cyan-800 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
            <h2 className="text-base font-bold">{isArabic ? "أوامر الاعتماد" : "Credential commands"}</h2>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              {view.canRegenerate ? (
                <button type="button" onClick={() => setDialog("regenerate")} disabled={!view.ownsSelectedServerState || view.binding.status !== "READY" || view.binding.hasStagedCandidate || Boolean(view.activeAction)} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-rose-700 px-4 text-sm font-bold text-white hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-50">
                  <RotateCcw className="size-4" aria-hidden="true" />{isArabic ? "تدوير كلمة المرور" : "Rotate password"}
                </button>
              ) : null}
              {view.canReconcile ? (
                <button type="button" onClick={() => setDialog("reconcile")} disabled={!view.ownsSelectedServerState || Boolean(view.activeAction)} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 px-4 text-sm font-bold hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:hover:bg-slate-900">
      ...
      >
        <label className="flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={rotationEnabled} onChange={(event) => setRotationEnabled(event.target.checked)} className="size-4" />{isArabic ? "تفعيل التدوير التلقائي" : "Enable automatic rotation"}</label>
...
function ReasonField({ value, onChange, isArabic }: { value: string; onChange: (value: string) => void; isArabic: boolean }) {
  return <label className="block text-xs font-bold text-slate-600 dark:text-slate-300">{isArabic ? "سبب موثق (8 أحرف على الأقل)" : "Audit reason (at least 8 characters)"}<textarea value={value} onChange={(event) => onChange(event.target.value)} maxLength={500} rows={3} className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 text-sm dark:border-slate-700 dark:bg-slate-900" /></label>;
}
