  ...
  return (
    <div ref={containerRef} className={`relative inline-block ${className}`}>
      ...
      <button
        ...
        className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 flex items-center justify-between gap-2 focus:outline-none focus:border-blue-600 transition-colors cursor-pointer min-w-[160px]"
      >
        <div className="flex items-center gap-1.5 truncate">
          {value === "ALL" || !value ? (
            <>
              <Globe className="w-3.5 h-3.5 text-blue-500 shrink-0" />
              <span className="truncate">{allowAll && value === "ALL" ? allLabel : placeholder}</span>
            </>
          ) : selectedCountry ? (
            <>
