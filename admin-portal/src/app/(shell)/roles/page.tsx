  ...
  return (
    <div className="space-y-6 w-full">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 text-2xs font-semibold uppercase tracking-wider bg-purple-500/20 text-purple-300 border border-purple-500/30 rounded-md">
                    RBAC Manifest
                  </span>
                </div>
                <p className="text-xs text-purple-100/80 mt-0.5 max-w-xl leading-tight">
            ...
            {canCreate && (
              <button
                ...
                className="px-3.5 py-2 text-xs font-semibold bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-400 hover:to-pink-400 text-white rounded-xl shadow-md transition-all flex items-center gap-1.5 cursor-pointer shrink-0 border border-white/20"
              >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="relative overflow-hidden bg-white dark:bg-slate-900/90 p-4 rounded-xl border border-slate-200/80 dark:border-indigo-500/20 shadow-sm hover:shadow-md transition-all">
            <div className="absolute top-0 end-0 w-20 h-20 bg-indigo-500/10 rounded-full blur-xl pointer-events-none" />
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
              <span className="text-xs font-semibold uppercase tracking-wider">{lang === "ar" ? "إجمالي الأدوار" : "Total Roles"}</span>
          ...
          <div className="relative overflow-hidden bg-white dark:bg-slate-900/90 p-4 rounded-xl border border-slate-200/80 dark:border-purple-500/20 shadow-sm hover:shadow-md transition-all">
            <div className="absolute top-0 end-0 w-20 h-20 bg-purple-500/10 rounded-full blur-xl pointer-events-none" />
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
              <span className="text-xs font-semibold uppercase tracking-wider">{lang === "ar" ? "أدوار النظام (المعروضة)" : "System Roles (Visible)"}</span>
              <div className="p-1.5 bg-purple-50 dark:bg-purple-950/50 text-purple-600 dark:text-purple-400 rounded-lg">
                <Lock className="w-4 h-4" />
              </div>
            </div>
            <div className="text-2xl font-semibold text-purple-600 dark:text-purple-400 font-mono mt-1">
          ...
          <div className="relative overflow-hidden bg-white dark:bg-slate-900/90 p-4 rounded-xl border border-slate-200/80 dark:border-emerald-500/20 shadow-sm hover:shadow-md transition-all">
            ...
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
              <span className="text-xs font-semibold uppercase tracking-wider">{lang === "ar" ? "أدوار مخصصة (المعروضة)" : "Custom Roles (Visible)"}</span>
          ...
          <div className="relative overflow-hidden bg-white dark:bg-slate-900/90 p-4 rounded-xl border border-slate-200/80 dark:border-pink-500/20 shadow-sm hover:shadow-md transition-all">
            <div className="absolute top-0 end-0 w-20 h-20 bg-pink-500/10 rounded-full blur-xl pointer-events-none" />
            <div className="flex items-center justify-between text-slate-500 dark:text-slate-400">
              <span className="text-xs font-semibold uppercase tracking-wider">{lang === "ar" ? "سياسات الوصول" : "Access Policies"}</span>
              <div className="p-1.5 bg-pink-50 dark:bg-pink-950/50 text-pink-600 dark:text-pink-400 rounded-lg">
                <ShieldCheck className="w-4 h-4" />
              </div>
            </div>
            <div className="text-2xl font-semibold text-pink-600 dark:text-pink-400 font-mono mt-1 flex items-center gap-2">
              Active
        ...
        {listError ? (
          <div role="alert" className="flex flex-col gap-3 rounded-xl border border-rose-200 bg-rose-50 p-4 text-xs text-rose-800 sm:flex-row sm:items-center sm:justify-between dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-200">
            ...
            <button type="button" onClick={refreshRoles} className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-300 px-3 py-2 font-semibold hover:bg-rose-100 dark:border-rose-800 dark:hover:bg-rose-900/50">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs overflow-hidden">
                  ...
                  roles.map((role) => (
                    ...
                    >
                      <td className="py-2.5 px-4">
                        <Link
                          href={`/roles/${role.id}`}
                          className="font-semibold text-xs text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1.5"
                        >
                          {role.name}
                        </Link>
                        {role.description && (
                      ...
                      <td className="py-2.5 px-4">
                        {role.isSystem ? (
                          <span className="flex items-center gap-1 text-xs text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md w-fit font-medium">
                            <Lock className="w-3 h-3" />
                            {lang === "ar" ? "نظام (للقراءة فقط)" : "System (Read-only)"}
                          </span>
                        ) : (
                          <span className="text-xs text-emerald-600 bg-emerald-50 border border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800 dark:text-emerald-400 px-2 py-0.5 rounded-md w-fit font-medium">
                            {lang === "ar" ? "مخصص" : "Custom"}
                          </span>
                        )}
                      </td>
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/50 p-3 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800 dark:bg-slate-800/30">
