...
export function OpportunityCard({ item, index, updateImportance }: OpportunityCardProps) {
      ...
      {(provided, snapshot) => (
        ...
        >
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-1 flex-1">
              <div
                ...
                className="mt-0.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity"
              >
                <GripVertical className="w-4 h-4" />
              </div>
              <div className="flex-1">
                <a href={`/crm/pipeline/${item.id}`} className="font-semibold text-sm text-gray-900 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400 line-clamp-2">
                  {item.title}
                </a>
          ...
          <div className="flex items-center justify-between mt-1 pt-2 border-t border-gray-100 dark:border-gray-800/60">
            <div className={cn("px-2 py-0.5 rounded text-[10px] font-bold flex items-center", getActivityColor(item.activityState))}>
              {getActivityIcon(item.activityState)}
              {item.activityState === "NO_OPEN" ? "لا يوجد نشاط" :
               item.activityState === "OVERDUE" ? "متأخر" :
               item.activityState === "TODAY" ? "اليوم" : "مستقبلي"}
            </div>
