...
const PERIODS_RESPONSE_LIMIT_BYTES = 120_000;
const PERIOD_RESPONSE_LIMIT_BYTES = 20_000;

type PendingAction = { id: string; action: "close" | "reopen" } | null;

/**
 * Periods are COMPANY-scoped and limit-only: the response is a bare array with
 * no `total`, so the screen shows what came back and says the list is capped
 * rather than rendering a pager it cannot honour.
 */
export function useInventoryPeriods() {
  ...
  const [items, setItems] = useState<InventoryPeriod[]>([]);
  const [statusFilter, setStatusFilter] = useState<PeriodStatus | undefined>(undefined);
  ...
  const load = useCallback(
    async (signal?: AbortSignal): Promise<void> => {
      ...
      setQueryError(null);
      try {
        const response = await tradeGet(periodsListPath(statusFilter), {
          signal,
          headers: scope.headers,
          maxResponseBytes: PERIODS_RESPONSE_LIMIT_BYTES,
        });
        setItems(parseTradeArray(response.data, parseInventoryPeriod, invalidResponse));
        setHasLoaded(true);
      } catch (error) {
  ...
  const create = useCallback(
    async (values: PeriodFormValues): Promise<boolean> => {
      if (!canManage || isSubmitting) return false;
      setIsSubmitting(true);
      setFormError(null);
      try {
        const response = await tradePost(INVENTORY_PERIODS_PATH, buildCreatePeriodRequest(values), {
          headers: scope.headers,
          maxResponseBytes: PERIOD_RESPONSE_LIMIT_BYTES,
        });
        setCreateOpen(false);
        toast.success(
          ...
          isTradeReplay(response.headers)
            ...
            : t.tradeInventory.periodCreated,
        );
        await load();
        return true;
      } catch (error) {
  ...
  const runTransition = useCallback(
    async (reason: string): Promise<void> => {
      ...
      try {
        // Both transitions answer 200 and both need If-Match; the reason is a
        // CODE matching ^[A-Z][A-Z0-9_.-]{1,79}$, not free text.
        await tradePost(periodActionPath(target.id, transition.action), buildReasonCode(reason), {
          headers: { ...scope.headers, "If-Match": tradeIfMatch(target.version) },
          maxResponseBytes: PERIOD_RESPONSE_LIMIT_BYTES,
        });
        toast.success(t.tradeCommon.savedTitle, t.tradeInventory.periodTransitioned);
        setTransition(null);
      } catch (error) {
        const normalized = normalizeApiError(error);
        if (normalized.status !== 403 && !toast.outcomeFromApi(normalized)) {
          toast.error(
            t.tradeInventory.periodTransitionFailed,
  ...
  return {
    ...
    openTransition: (period: InventoryPeriod, action: "close" | "reopen") =>
      setTransition({ id: period.id, action }),
    closeTransition: () => {
      if (pending) return;
      setTransition(null);
    },
