...
export default function PipelineGeneralPage() {
    const { t } = useI18n();
  const [name, setName] = useState(t.crm.directSalesFunnelForHospit);
  const [code, setCode] = useState("b2b_enterprise");
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-6">
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">{t.crm.updatePathNameAndCodeInst}</p>
        </div>
        {isSaved && (
      ...
      <form onSubmit={handleInlineSave} className="space-y-4 max-w-2xl">
        <Input label={t.crm.salesFunnelName} value={name} onChange={(e) => setName(e.target.value)} required />
