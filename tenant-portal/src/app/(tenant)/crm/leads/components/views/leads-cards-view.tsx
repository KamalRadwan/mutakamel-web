...
export function LeadsCardsView({ items, onDelete }: LeadsCardsViewProps) {
  const { lang } = useI18n();
  const isRtl = lang === "ar";
  ...
  if (items.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-slate-500">
        <p className="text-lg font-medium">{isRtl ? "I18N_FALLBACK" : "No Leads Found"}</p>
        <p className="text-sm mt-1">{isRtl ? "I18N_FALLBACK" : "Try changing search terms"}</p>
      </div>
    );
  }
