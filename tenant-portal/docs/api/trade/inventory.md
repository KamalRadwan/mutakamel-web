...
> Tenant Portal status: `tenant-portal` replaces the legacy tenant Trade UI; Legacy `mutakamel-web-app` has live inventory operations/governance clients and UI. The replacement `tenant-portal` is not implemented.
