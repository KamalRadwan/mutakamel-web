# Trade Invoices and Contracts API
...
> Controller-relative prefixes: `/trade/invoices`, `/trade/contracts`
> Tenant Portal status: `tenant-portal` replaces the legacy tenant Trade UI; No dedicated invoice/contract API client was found in the legacy Trade frontend. The replacement `tenant-portal` is not implemented.

## Capability

Invoice and contract draft creation, mutation, finalization/activation, and branch-scoped projections.
...
- `../backend/mutakamel-apps/trade-app/src/modules/financial-documents/financial-documents.service.ts`
- `../backend/mutakamel-apps/trade-app/src/modules/financial-documents/financial-documents.controller.spec.ts`
- `../backend/mutakamel-apps/trade-app/src/modules/financial-documents/financial-documents.service.spec.ts`
...
|---|---|---|---|---|---|---|---|---|
| GET | `/api/tenant/trade/v1/invoices` | `/trade/invoices` | `trade.invoices.read` | `BRANCH` | `query: DocumentListQueryDto` | — | 200 | `AUTHENTICATED`; gateway-idempotent=true |
| POST | `/api/tenant/trade/v1/invoices` | `/trade/invoices` | `trade.invoices.create` | `BRANCH` | `body: CreateInvoiceDto` | `X-Idempotency-Key` | 201 | `WRITE_SENSITIVE`; gateway-idempotent=true |
| GET | `/api/tenant/trade/v1/invoices/:id` | `/trade/invoices/:id` | `trade.invoices.read` | `BRANCH` | — | — | 200 | `AUTHENTICATED`; gateway-idempotent=true |
| PATCH | `/api/tenant/trade/v1/invoices/:id` | `/trade/invoices/:id` | `trade.invoices.update` | `BRANCH` | `body: UpdateInvoiceDto` | `X-Idempotency-Key`, `If-Match` | 200 | `WRITE_SENSITIVE`; gateway-idempotent=true |
| POST | `/api/tenant/trade/v1/invoices/:id/issue` | `/trade/invoices/:id/issue` | `trade.invoices.issue` | `BRANCH` | — | `X-Idempotency-Key`, `If-Match` | 200 | `WRITE_SENSITIVE`; gateway-idempotent=true |
| GET | `/api/tenant/trade/v1/contracts` | `/trade/contracts` | `trade.contracts.read` | `BRANCH` | `query: DocumentListQueryDto` | — | 200 | `AUTHENTICATED`; gateway-idempotent=true |
| POST | `/api/tenant/trade/v1/contracts` | `/trade/contracts` | `trade.contracts.create` | `BRANCH` | `body: CreateContractDto` | `X-Idempotency-Key` | 201 | `WRITE_SENSITIVE`; gateway-idempotent=true |
| GET | `/api/tenant/trade/v1/contracts/:id` | `/trade/contracts/:id` | `trade.contracts.read` | `BRANCH` | — | — | 200 | `AUTHENTICATED`; gateway-idempotent=true |
| PATCH | `/api/tenant/trade/v1/contracts/:id` | `/trade/contracts/:id` | `trade.contracts.update` | `BRANCH` | `body: UpdateContractDto` | `X-Idempotency-Key`, `If-Match` | 200 | `WRITE_SENSITIVE`; gateway-idempotent=true |
| POST | `/api/tenant/trade/v1/contracts/:id/activate` | `/trade/contracts/:id/activate` | `trade.contracts.activate` | `BRANCH` | — | `X-Idempotency-Key`, `If-Match` | 200 | `WRITE_SENSITIVE`; gateway-idempotent=true |
...
- All IDs use UUIDv7; currency uses three uppercase letters; financial values are decimal strings. Nested invoice lines/totals and contract content are validated and bounded.
- Invoice line classification is `FINANCIAL` or `NON_FINANCIAL`. Issue/activate has no request body but requires concurrency/idempotency headers.
- `DocumentListQueryDto.status` is free-form at transport validation.

- Invoice line classification: `FINANCIAL`, `NON_FINANCIAL`.
- Lifecycle status values are service projections; do not derive a closed request enum from TypeScript response interfaces.
...
- Domain error codes observed in the owning module: `TRADE.CATALOG.UOM_INVALID`, `TRADE.CONCURRENCY.STALE_VERSION`, `TRADE.CONTEXT.MISSING_COMPANY`, `TRADE.CONTRACT.EVIDENCE_INVALID`, `TRADE.CONTRACT.NOT_FOUND`, `TRADE.CONTRACT.TRANSITION_NOT_ALLOWED`, `TRADE.FINALIZED_DOCUMENT.NUMBERING_UNAVAILABLE`, `TRADE.FINALIZED_DOCUMENT.SNAPSHOT_INCOMPLETE`, `TRADE.INVOICE.CUSTOMER_INVALID`, `TRADE.INVOICE.FINANCIAL_EVIDENCE_INVALID`, `TRADE.INVOICE.LINE_INVALID`, `TRADE.INVOICE.NOT_FOUND`, `TRADE.INVOICE.TRANSITION_NOT_ALLOWED`.

- Create returns 201; updates and issue/activate return 200. Every write requires idempotency; mutable/finalizing operations require `If-Match`.
- All routes are branch-scoped. Finalization captures immutable legal/financial evidence; PDF rendering is a separate asynchronous contract in `pdf-render-jobs.md`.
...
const result = await tenantTradeApi.request(
  "/api/tenant/trade/v1/invoices/019f98a0-1234-7abc-8def-1234567890ab",
  {
    method: "GET",
  headers: {
...
- Legacy frontend parity is absent for these routes. Treat every UI workflow as new replacement work and verify response shapes from the API.
