...
> Tenant Portal status: `tenant-portal` replaces the legacy tenant Trade UI; Legacy `mutakamel-web-app` has a live dashboard workspace, builder, API client, route pages, and tests. The replacement `tenant-portal` is not implemented.
