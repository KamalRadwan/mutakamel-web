...
> Tenant Portal status: `tenant-portal` replaces the legacy tenant Trade UI; Legacy `mutakamel-web-app` has live quotation and sales-order clients/screens, including quotation detail and confirmation polling. The replacement `tenant-portal` is not implemented.
